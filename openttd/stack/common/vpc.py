...
class VpcStack(Stack):
    def __init__(self, scope: Construct, id: str, **kwargs) -> None:
        ...
        nat_provider = NatProvider.instance(instance_type=InstanceType("t3a.nano"))
        ...
        for index, subnet in enumerate(self._vpc.public_subnets):
            ...
            subnet.node.children[0].ipv6_cidr_block = Fn.select(
                index, Fn.cidr(Fn.select(0, self._vpc.vpc_ipv6_cidr_blocks), len(self._vpc.public_subnets), "64")
            )
